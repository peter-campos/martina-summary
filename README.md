# 📱 El Viaje de Martina

> **Conoce tus derechos. Tu cuerpo, tu decisión.**

[![Flutter](https://img.shields.io/badge/Flutter-02569B?style=for-the-badge&logo=flutter&logoColor=white)](https://flutter.dev)
[![Dart](https://img.shields.io/badge/Dart-0175C2?style=for-the-badge&logo=dart&logoColor=white)](https://dart.dev)
[![Android](https://img.shields.io/badge/Android-APK-3DDC84?style=for-the-badge&logo=android&logoColor=white)](https://github.com/peter-campos/martina-summary/releases/tag/v1.1.5)

---

## 📌 About

**El Viaje de Martina** is an educational mobile app developed in collaboration with **Universidad de los Andes** (Colombia) and the **University of Southampton** (UK), designed to inform migrant women about their sexual and reproductive rights in an accessible, offline-first format.

The app was published on the **App Store (May 20, 2024)** and **Google Play (May 11, 2024)**. The project agreement with the client has since ended and the app is no longer available in the stores. With Coolearning Consulting LLC's permission, the app is distributed directly for demonstration purposes.

---

## ✨ Features

- 📖 Informational content on sexual and reproductive rights for migrant women
- 🎙️ Embedded podcast player with offline support
- 📵 Fully offline — no internet connection required after installation
- 🌐 Available in Spanish

---

## 🛠️ Stack

| | |
|---|---|
| Framework | Flutter |
| Language | Dart |
| Target | Android, iOS |

Flutter and Dart were chosen to fit the client's budget and simplify long-term maintenance with a single shared codebase for both platforms.

---

## 📥 Download & Try It

### Android
[![Request Access](https://img.shields.io/badge/Request_Access-3DDC84?style=for-the-badge&logo=android&logoColor=white)](https://appdistribution.firebase.dev/i/a6d5149c83671c3a)

Register your email via the link above to receive the APK download link directly to your inbox. Alternatively, download the APK directly from [GitHub Releases](https://github.com/peter-campos/martina-summary/releases/tag/v1.1.5) and verify the file integrity:
```
SHA1: 4c23a772940fca8e82b285b240b91e982cfce3fb
```

To install: enable **Install from unknown sources** on your Android device, then open the downloaded file.

### iOS
To test the iOS build via TestFlight, send an email to [peter.campos.leon@gmail.com](mailto:peter.campos.leon@gmail.com) with the subject **"El Viaje de Martina - TestFlight Access"** and you will be added manually.

---

## 📸 Screenshots

<img src="https://github.com/user-attachments/assets/44fa9f22-c64e-4e71-af57-1d4403543e05" width="300"/>
[![El Viaje de Martina - Demo](https://img.youtube.com/vi/Fd_Ny3GoZ80/0.jpg)](https://www.youtube.com/shorts/Fd_Ny3GoZ80)

---

## 📄 License

Developed under contract for Coolearning Consulting LLC. Distributed for portfolio demonstration purposes with permission.

---

## 📬 Contact

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Peter%20Campos-0077B5?style=for-the-badge&logo=linkedin)](https://www.linkedin.com/in/peter-campos)
[![Email](https://img.shields.io/badge/Email-peter.campos.leon%40gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:peter.campos.leon@gmail.com)
